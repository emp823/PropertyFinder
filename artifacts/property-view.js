import React, { Component } from 'react';
import { StyleSheet, Image, View, Text } from 'react-native';
const styles = StyleSheet.create({
    container: {
        marginTop: 65
    },
    heading: {
        backgroundColor: '#F8F8F8'
    },
    separator: {
        height: 1,
        backgroundColor: '#DDDDDD'
    },
    image: {
        width: 400,
        height: 300
    },
    price: {
        fontSize: 25,
        margin: 5,
        color: '#48BBEC'
    },
    title: {
        fontSize: 20,
        margin: 5,
        color: '#656565'
    },
    description: {
        fontSize: 18,
        margin: 5,
        color: '#656565'
    }
});
export class PropertyView extends Component {
    render() {
        const property = this.props.property;
        let stats = `${property.bedroom_number} bed ${property.property_type}`;
        if (property.bathroom_number) {
            stats += `, property.bathroom_number ${property.bathroom_number > 1 ? 'bathrooms' : 'bathroom'}`;
        }
        const price = property.price_formatted.split(' ')[0];
        return (React.createElement(View, { style: styles.container },
            React.createElement(Image, { style: styles.image, source: { uri: property.img_url } }),
            React.createElement(View, { style: styles.heading },
                React.createElement(Text, { style: styles.price }, price),
                React.createElement(Text, { style: styles.title }, property.title),
                React.createElement(View, { style: styles.separator })),
            React.createElement(Text, { style: styles.description }, stats),
            React.createElement(Text, { style: styles.description }, property.summary)));
    }
}
//# sourceMappingURL=property-view.js.map